...
const GuideEtage = () => {
  ...
  useEffect(() => {
    ...
    if (storedState) {
      ...
      setForagePilote(parsedState.foragePilote || false);
      setFullGuide(parsedState.fullGuide || false);
      setCost(parsedState.cost || 450);
      setOriginalCost(parsedState.originalCost || 450);
      setSelectSurgicalKitBrand(parsedState.selectSurgicalKitBrand || "");
      setImplantBrandInputs(parsedState.implantBrandInputs || []);
      setImmediateLoad(parsedState.immediateLoad || false);
      setSecondSwitch(parsedState.secondSwitch || false);
      setThirdSwitch(parsedState.thirdSwitch || false);
      setFourthSwitch(parsedState.fourthSwitch || false);
      setFifthSwitch(parsedState.fifthSwitch || false);
      setSmileDesign(parsedState.smileDesign || false);
      setComment(parsedState.comment || "");
      setImplantBrandValues(parsedState.implantBrandValues || {});
      setLateralPinBrand(parsedState.lateralPinBrand || "")
    }
  ...
  const handleNextClick = () => {
    ...
    localStorage.setItem("guideEtageState", JSON.stringify(store));
    navigate("/SelectedItemsPageGETAGE", {
      state: {
        selectedItemsData: yourData,
        previousStates: {
  ...
  // console.log("wwwwwww",location.state.selectedTeeth)
  useEffect(() => {
    // Check if location.state exists and has selectedTeeth
    if (location.state && location.state.selectedTeeth) {
      setSelectedTeeth(location.state.selectedTeeth);

    } else {

      console.error("No selectedTeeth found in location.state:", location.state);
      // Navigate or handle the error as per your application's logic
      // navigate('/error'); // Example: navigate to an error page
    }
  }, [location.state]);
  const selectedTeethData = location.state && location.state.selectedTeeth ? location.state.selectedTeeth : [];
